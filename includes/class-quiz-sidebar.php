    ...
    public function enqueue_scripts_styles() {
        if (is_singular('sfwd-quiz')) {
            ...
            if ($enforce_hint === '1') {
                ...
                // Enqueue JavaScript for hint enforcement - REPAIRED VERSION
                $script_handle = 'lilac-quiz-hint-enforcement-repaired';
                wp_register_script(
                    $script_handle,
                    LILAC_QUIZ_SIDEBAR_PLUGIN_URL . 'assets/js/quiz-hint-enforcement-repaired.js',
                    array('jquery'),
                    LILAC_QUIZ_SIDEBAR_VERSION . '-repaired-' . time(),
                    true
                );
                
                // Add debug information
                wp_add_inline_script($script_handle, 
                    'console.log("Lilac Quiz: REPAIRED Hint Enforcement script registered");',
                    'before'
                );
                ...
                add_action('wp_footer', function() use ($script_handle) {
                    ...
                    document.addEventListener("DOMContentLoaded", function() {
                        ...
                        // Check if our REPAIRED script is in the DOM
                        const scriptElements = document.getElementsByTagName("script");
                        let scriptFound = false;
                        for (let i = 0; i < scriptElements.length; i++) {
                            if (scriptElements[i].src.includes("quiz-hint-enforcement-repaired.js")) {
                                scriptFound = true;
                                console.log("Found quiz-hint-enforcement-repaired.js in the DOM");
                                break;
                            }
                        }
                        
                        if (!scriptFound) {
                            console.error("quiz-hint-enforcement-repaired.js not found in the DOM!");
                            
                            // Try to load it manually
                            const script = document.createElement("script");
                            script.src = "' . LILAC_QUIZ_SIDEBAR_PLUGIN_URL . 'assets/js/quiz-hint-enforcement-repaired.js?ver=' . LILAC_QUIZ_SIDEBAR_VERSION . '-repaired";
                            script.onload = function() {
                                console.log("Successfully loaded REPAIRED quiz script manually!");
                            };
                            script.onerror = function() {
                                console.error("Failed to load REPAIRED quiz script!");
                            };
